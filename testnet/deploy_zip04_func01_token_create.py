...
token_create_sourcecode = open('../funcs/token_create.py', 'r').read()
...
if __name__ == '__main__':
    ...
    call = {'p': setting.protocol,
            'f': 'function_proposal',
            'a': ['token_create', token_create_sourcecode, [], [], []]}
    transaction = {
    ...
    call = {'p': setting.protocol,
            'f': "function_vote", 
            'a': ['token_create', hashlib.sha256(token_create_sourcecode.encode('utf8')).hexdigest()]}
    transaction = {
        'from': account.address,
        'to': account.address,
        'value': 0,
