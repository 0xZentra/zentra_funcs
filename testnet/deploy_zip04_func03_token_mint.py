...
token_mint_sourcecode = open('../funcs/token_mint.py', 'r').read()
...
if __name__ == '__main__':
    ...
    call = {'p': setting.protocol,
            'f': 'function_proposal',
            'a': ['token_mint', token_mint_sourcecode, [], [], []]}
    transaction = {
    ...
    call = {'p': setting.protocol,
            'f': "function_vote", 
            'a': ['token_mint', hashlib.sha256(token_mint_sourcecode.encode('utf8')).hexdigest()]}
    transaction = {
        'from': account.address,
        'to': account.address,
        'value': 0,
