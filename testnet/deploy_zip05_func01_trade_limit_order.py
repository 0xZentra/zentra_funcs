...
trade_limit_order_sourcecode = open('../funcs/trade_limit_order.py', 'r').read()
...
if __name__ == '__main__':
    ...
    call = {'p': setting.protocol,
            'f': 'function_proposal',
            'a': ['trade_limit_order', trade_limit_order_sourcecode, [], [], []]}
    transaction = {
    ...
    call = {'p': setting.protocol,
            'f': "function_vote", 
            'a': ['trade_limit_order', hashlib.sha256(trade_limit_order_sourcecode.encode('utf8')).hexdigest()]}
    transaction = {
        'from': account.address,
        'to': account.address,
        'value': 0,
